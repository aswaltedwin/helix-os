  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setNewAgent({ name: "", category: "finance", description: "", system_prompt: "You are a helpful specialist agent for HelixOS." });
        fetchAgents();
      }
    } catch (err) {
        ...
        {agents.map((agent) => (
          ...
          >
            ...
            <div className="flex justify-between items-center pt-6 border-t border-gray-50">
              ...
              <button className="text-indigo-600 text-sm font-bold hover:underline">Configure →</button>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-[32px] w-full max-w-lg shadow-2xl overflow-hidden animate-in slide-in-from-bottom-8 duration-300">
            <div className="p-8 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <h3 className="text-2xl font-black text-gray-900">New Specialist</h3>
